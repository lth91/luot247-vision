-- Update reset function to keep all logs of current month for chart display
...
BEGIN
  ...
  -- Step 6: Delete ONLY logs from previous months (keep all logs of current month for chart)
  -- This keeps logs for all days in current month to display on monthly chart
  DELETE FROM view_logs2 WHERE viewed_at < v_month_start;
  
  RAISE NOTICE 'Reset completed successfully. Kept all logs from current month (since %).', v_month_start;
END;
$function$;