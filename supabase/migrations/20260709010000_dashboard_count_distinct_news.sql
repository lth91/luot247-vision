-- Bảng theo dõi gửi tin: cột "Tin đăng hôm nay/tháng này" đổi từ đếm LƯỢT CHẤM
-- sang đếm TIN THẬT (gom theo tiêu đề, không phân biệt hoa thường).
--
-- Lý do (phàn nàn của nhân viên 09/07): 1 tin bị AI loại → sửa → import lại
-- tạo 2-3 dòng submission_log, cột "Tin đăng" phồng lên so với số tin thực tế
-- (vd 114 lượt nhưng chỉ ~100 tin), trong khi tin trùng bị bỏ qua lại không
-- tính. Đếm DISTINCT tiêu đề khớp trực giác "tôi đã gửi bao nhiêu tin".
--
-- Ghi chú: nếu bản sửa ĐỔI CẢ TIÊU ĐỀ thì vẫn tính 2 tin (tiêu đề khác nhau
-- là tin khác nhau — chấp nhận). Dòng log không có tiêu đề (dữ liệu cũ) đếm
-- riêng từng dòng qua fallback id. Cột "Tin duyệt" giữ nguyên COUNT thường
-- (tin duyệt không bao giờ bị ghi trùng nhờ dedup trigram trước khi đăng).
