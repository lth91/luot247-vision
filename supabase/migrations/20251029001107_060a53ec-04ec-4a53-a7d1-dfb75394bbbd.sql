...
-- Function to reset daily view stats at 7 AM GMT+7
...
BEGIN
  ...
  -- Step 6: Delete old logs (keep only today's logs)
  DELETE FROM view_logs2 WHERE viewed_at < v_today_7am;
  
  RAISE NOTICE 'Reset completed successfully';
END;
$function$;

-- Schedule cron job to run daily at 7:00 AM GMT+7 (which is 00:00 UTC)
SELECT cron.schedule(
  'reset-daily-view-stats2',
  '0 0 * * *', -- Every day at 00:00 UTC (7:00 AM GMT+7)
  $$
  SELECT public.reset_daily_view_stats2();
  $$
);

-- Run the function immediately to reset for today
SELECT public.reset_daily_view_stats2();