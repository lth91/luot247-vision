...
-- Fix reset_daily_view_stats2 to properly preserve yesterday's value
...
BEGIN
  ...
  -- CRITICAL FIX: Get current "today" stat_value from view_stats2 table
  -- This is yesterday's COMPLETE count including both base + logs
  SELECT COALESCE(
    (SELECT stat_value FROM view_stats2 WHERE stat_key = 'today'),
    0
  ) INTO v_current_today_value;
  
  -- Log current values before reset
  RAISE NOTICE 'Resetting stats at % (GMT+7)', now() AT TIME ZONE 'Asia/Ho_Chi_Minh';
  RAISE NOTICE 'Moving today stat_value % to yesterday', v_current_today_value;
  
  -- Step 1: Move today's stat_value → yesterday (this preserves the COMPLETE count)
  UPDATE view_stats2 
  ...
  -- Step 2: Reset today base to 0 (will accumulate new logs from 7 AM onwards)
  UPDATE view_stats2 
  SET stat_value = 0, 
      updated_at = now(),
      last_reset_at = now()
